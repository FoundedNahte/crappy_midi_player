...
class Parser:
    ...
    def event_midi(self, evt, track_number):
        evt_type = evt >> 4
        channel = evt & 0x0F
        ...
        if evt_type == 0x8:  # Note Off
            ...
            self.notes[track_number].append([np.array([note]), vel, start_time, self.time])
        ...
        for i_track in range(self.n_tracks):
            ...
            while self.file.tell() - init_pos < track_len and not self.end_of_track:
                ...
                if evt < 0x80:
                    ...
                    self.move(-1)
                    self.event_midi(prev_status, i_track)
                elif evt < 0xF0:  # MIDI event
                    self.event_midi(evt, i_track)
                    prev_status = evt
